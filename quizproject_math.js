...
function fetchQuestion() {
  let rand = Math.floor(Math.random() * 100000);
	fetch("https://opentdb.com/api.php?amount=10&category=19&type=multiple&foo=" + rand)
...
function markIt(ans) {
    ...
    message += "Final Score: " + score + " / " + questions.length;
	   ...
	   message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='sport()'>Next Quiz: Sports</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft < 0){
	...
	+ "<button id='good' onclick='sport()'>Next Quiz: Sports</button>";
