...
function fetchQuestion() {
  ...
  fetch("https://opentdb.com/api.php?amount=10&category=11&type=multiple&foo=" + rand)
...
function changeData(data) {
   ...
   },
   {
	...
	"question_number": "9",
	"a": data.results[8].incorrect_answers[0],
	"b": data.results[8].correct_answer,
...
function markIt(ans) {
    ...
    if (currentQuestion >= questions.length) {
       ...
       if (ans == questions[9].answer) {
         message = "Correct! Your score is " + score + " / " + questions.length + "<br><br>";
         document.getElementById("message").style.backgroundColor = "green";
       } else {
         message = "Incorrect! Your score is " + score + " / " + questions.length + "<br>" + "Correct Answer: " + questions[9].answer + ") " + questions[9].c_answer + "<br><br>";
         document.getElementById("message").style.backgroundColor = "red";
       }
	   ...
	   if(score == 0 || score == 1) {
		...
		message += "<br><br>" + "This is not bad, but it is still not good. Keep working on it!";
	   ...
	   message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='vg()'>Next Quiz: Video Game</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft <= -1){
	...
	+ "<button id='good' onclick='vg()'>Next Quiz: Video Game</button>";
