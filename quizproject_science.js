...
function fetchQuestion() {
  let rand = Math.floor(Math.random() * 100000);
	fetch("https://opentdb.com/api.php?amount=10&category=17&type=multiple&foo=" + rand)
    ...
    if (currentQuestion >= questions.length) {
     ...
     message += "Final Score: " + score + " / " + questions.length;
	   ...
	   if(score == 0 || score == 1) {
	    ...
	    message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='math()'>Next Quiz: Math</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft <= -1){
	...
	+ "<button id='good' onclick='math()'>Next Quiz: Math</button>";
