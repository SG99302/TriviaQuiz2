...
function fetchQuestion() {
  let rand = Math.floor(Math.random() * 100000);
	fetch("https://opentdb.com/api.php?amount=10&category=9&type=multiple&foo=" + rand)
    ...
    // move to the next question
      ...
      // create a special message
	   ...
	   if(score == 0 || score == 1) {
	    ...
	    message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='film()'>Next Quiz: Film</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft <= -1){
	...
	+ "<button id='good' onclick='film()'>Next Quiz: Film</button>";
