...
function fetchQuestion() {
  let rand = Math.floor(Math.random() * 100000);
	fetch("https://opentdb.com/api.php?amount=10&category=21&type=multiple&foo=" + rand)
 ...
 function markIt(ans) {
    ...
    if (currentQuestion >= questions.length) {
      ...
      message += "Final Score: " + score + " / " + questions.length;
	   ...
	   message += "<br><br>" + "What would you like to do next?" + "<br>" + "<button id='retry' onclick='retry()'>Play Again</button>" + "&nbsp;" + "<button id='cool' onclick='home()'>Go to Home</button>" + "&nbsp;" + "<button id='good' onclick='gk()'>Next Quiz: General Knowledge</button>";
    } else {
       loadQuestion();
    }
...
function timer() {
  ...
  if(timeleft <= -1){
	...
	+ "<button id='good' onclick='gk()'>Next Quiz: General Knowledge</button>";
